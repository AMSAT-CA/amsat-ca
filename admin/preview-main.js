          ...
          sections
            ? sections.map(function (section, index) {
                ...
                switch (sectionType) {
                  // Content Components
                  case "text-section":
                    return window.ContentPreviewComponents?.renderTextSection(section, index) || 
                           this.renderFallback(sectionType, index);
                  case "image":
                    return window.ContentPreviewComponents?.renderImageSection(section, index, getAsset) || 
                           this.renderFallback(sectionType, index);
                  case "split":
                    return window.ContentPreviewComponents?.renderSplitSection(section, index, getAsset) || 
                           this.renderFallback(sectionType, index);

                  // Interactive Components
                  case "hero":
                    return window.InteractivePreviewComponents?.renderHeroSection(section, index, getAsset) || 
                           this.renderFallback(sectionType, index);
                  case "cta":
                    return window.InteractivePreviewComponents?.renderCtaSection(section, index, getAsset) || 
                           this.renderFallback(sectionType, index);
                  case "featured_cards":
                    return window.InteractivePreviewComponents?.renderFeaturedCardsSection(section, index, getAsset) || 
                           this.renderFallback(sectionType, index);

                  // Social & Form Components
                  case "team":
                  case "team_members":
                    return window.SocialFormPreviewComponents?.renderTeamSection(section, index, getAsset) || 
                           this.renderFallback(sectionType, index);
                  case "social_links":
                    return window.SocialFormPreviewComponents?.renderSocialLinksSection(section, index) || 
                           this.renderFallback(sectionType, index);
                  case "contact_form":
                    return window.SocialFormPreviewComponents?.renderContactFormSection(section, index) || 
                           this.renderFallback(sectionType, index);
                  case "contact_form_socials":
                    return window.SocialFormPreviewComponents?.renderContactFormSocialsSection(section, index) || 
                           this.renderFallback(sectionType, index);
